---
title: "개요"
description: "다양한 식별자를 사용해 기업, 펀드, 암호화폐 로고를 검색 및 조회하는 종합 가이드"
---
...
Benzinga Logos API는 기업, 펀드, 암호화폐에 대한 고품질 로고에 접근할 수 있는 기능을 제공합니다. 심볼, CIK 번호(SEC 기업 식별자), 국제증권식별번호(ISIN) 코드, CUSIP, FIGI 등 여러 종류의 식별자를 사용하여 로고를 검색할 수 있습니다.
...
여러 식별자를 사용하여 로고를 검색합니다. 로고 URL, 종목 정보 및 메타데이터를 반환합니다.

<div id="logossync">
  ### `/logos/sync`
</div>

페이지네이션을 지원하며 여러 로고를 대량으로 동기화할 수 있는 엔드포인트입니다.

<div id="search-filters">
  ## 검색 필터
</div>

API는 다음 두 가지 주요 매개변수를 통해 유연한 검색을 지원합니다:

- `search_keys` - 검색하려는 식별자 값
- `search_keys_type` - 식별자 유형 (선택적; 기본값은 `symbol`)
...
SEC의 Central Index Key(CIK)를 사용해 검색합니다. 패딩된 값과 패딩되지 않은 값을 모두 지원합니다.
...
블룸버그의 금융상품 글로벌 식별자(Financial Instrument Global Identifier, FIGI)를 사용해 주식 클래스를 검색합니다.

**파라미터:**

* `search_keys`: FIGI 주식 클래스 ID (예: `BBG001SQKGD7`)
...
Committee on Uniform Securities Identification Procedures 번호를 사용하여 검색합니다.
...
콜론 (`:`) 또는 마침표 (`.`)를 구분자로 사용하여 심볼에 국가 코드 또는 통화 코드를 추가합니다.

**파라미터:**

* `search_keys`: 한정자가 포함된 심볼(예: `AAPL:US`, `AAPL:USD`, `AAPL.US`)
...
거래소 코드와 심볼을 조합해 검색합니다. Bloomberg 스타일(레거시)과 LSEG 스타일 형식 모두를 지원합니다.

**Bloomberg 스타일 매핑(레거시):**
...
ISO Market Identifier Code와 심볼을 조합하여 검색합니다.
...
여러 형식을 사용하여 암호화폐 로고를 검색할 수 있습니다. `search_keys_type`를 지정하지 않아도 API는 암호화폐 형식을 자동으로 인식합니다.
...
`fields` 매개변수를 사용해 특정 로고 유형을 요청할 수 있습니다. 사용 가능한 필드는 다음과 같습니다:
...
* `mark_composite_light` - 라이트 테마 합성 마크 (PNG)
* `mark_composite_dark` - 다크 테마 합성 마크 (PNG)
* `mark_vector_composite_light` - 라이트 테마 벡터 합성 마크 (SVG)
* `mark_vector_composite_dark` - 라이트 테마 벡터 합성 마크 (SVG)
...
- `composite_auto` - 합성 이미지를 자동으로 생성할지 여부를 나타내는 불리언 값
...
- `page` - 페이지 번호(정수)
- `pagesize` - 페이지당 결과 수(정수)
- `updated_since` - 업데이트 시각 기준 필터링(ISO 8601 형식)

<div id="security-details">
  ### 증권 상세 정보
</div>

- `securities` - 응답에 증권 상세 정보를 포함할지 여부 (boolean)
...
모든 Logos API 엔드포인트는 API 키 인증이 필요합니다. 토큰을 쿼리 매개변수로 포함하세요:
...
1. **search_keys_type를 지정** - API의 기본값은 `symbol`이지만, 유형을 명시적으로 지정하면 가독성과 성능이 향상됩니다
2. **필요한 필드만 요청** - 응답 크기를 최소화하고 성능을 향상하기 위해 필요한 로고 유형만 요청하십시오
3. **대용량 데이터셋에는 `bulk-sync` 사용** - 여러 로고를 조회할 때는 페이지네이션과 함께 `/logos/bulk-sync` 엔드포인트를 사용하십시오
4. **응답 캐싱** - 로고 url에는 만료 타임스탬프가 포함되어 있으므로, 적절히 캐싱하여 API 호출을 줄이십시오
5. **오류를 안정적으로 처리** - 항상 응답에서 `ok` 필드를 확인하고 오류 케이스를 적절히 처리하십시오

<div id="rate-limits">
  ## 요청 한도
</div>

요청 한도에 대한 자세한 내용은 사용 중인 API 구독 플랜을 참조하십시오. 애플리케이션에 더 높은 요청 한도가 필요한 경우 지원팀에 문의하십시오.

<div id="support">
  ## 지원
</div>

Logos API에 대해 추가적인 도움이 필요하시거나 질문이 있는 경우 Benzinga API 지원팀에 문의하거나 상세 엔드포인트 문서를 참고하세요.